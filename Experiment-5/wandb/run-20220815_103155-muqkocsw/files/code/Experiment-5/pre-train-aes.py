...
def compute_metrics(model_outputs, correct, prompt):
    ...
    model_outputs = [(output * (maximum-minimum)+minimum) for output in model_outputs]
    correct = [(output * (maximum-minimum)+minimum) for output in correct]
...
if __name__ == "__main__":
    ...
    run = wandb.init(project="AES-Experiment-9", name=f"{model}-prompt-{prompt}", config=config)
    train(prompt, model)
    run.finish()

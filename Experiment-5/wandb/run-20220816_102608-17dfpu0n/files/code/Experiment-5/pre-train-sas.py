...
def compute_metrics(model_outputs, correct):
    ...
    model_outputs = [int(output*3) for output in model_outputs]
    correct = [int(output*3) for output in correct]

    qwk = metrics.cohen_kappa_score(model_outputs, correct, weights='quadratic')
...
def train(technique=None, prompt=0):
    train_df, eval_df = load_data(f"datasets/{name}/data_prompt_{prompt}.csv")
...
if __name__ == "__main__":
    ...
    run = wandb.init(project="AES-Experiment-10", name=f"Self-Attention-{prompt}", config=config)
    train(technique='min_max', prompt=prompt)
    run.finish()
